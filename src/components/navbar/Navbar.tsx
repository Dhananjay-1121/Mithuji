 ...
 */}
      ...
      {localStorage.getItem("auth_token") && (
        <>
          <div className="d1" onClick={sellNavigate}>
            <CabinIcon className="icon" /> Sell
          </div>
          <div className="d2" onClick={() => navigate("/message")}>
            <SmsIcon className="icon" /> Messages
          </div>
        </>
      )}
      {user ? (
        <div className="d3" onClick={() => navigate("/customer-profile")}>
          <PermIdentityIcon className="icon" />
          <p className="text">{!user ? "Sign in" : `${user.name}`}</p>
        </div>
      ) : (
        <div className="d3" onClick={() => navigate("/login")}>
          <PermIdentityIcon className="icon" />
